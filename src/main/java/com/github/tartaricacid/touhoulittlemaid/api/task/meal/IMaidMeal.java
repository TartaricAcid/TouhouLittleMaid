...
public interface IMaidMeal {
    ArrayList<ResourceLocation> itemList = getMatchedList(MaidConfig.MAID_HEAL_MEALS_BLOCK_LIST_MATCH.get());
    ...
    static boolean isBlockListMatch(ItemStack food) {
        ResourceLocation key = ForgeRegistries.ITEMS.getKey(food.getItem());
    ...
    private static @NotNull ArrayList<ResourceLocation> getMatchedList(String match) {
        Pattern pattern = Pattern.compile(match);
        HolderLookup.RegistryLookup<Item>  items = BuiltInRegistries.ITEM.asLookup();
        return items.listElementIds()
