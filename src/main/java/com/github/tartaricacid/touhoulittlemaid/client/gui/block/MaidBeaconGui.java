...
public class MaidBeaconGui extends Screen {
    ...
    private void addStorageAndTakeButton() {
        this.addRenderableWidget(new Button(leftPos + 118, topPos + 72, 76, 20, Component.translatable("gui.touhou_little_maid.maid_beacon.add_one"), b -> {
            NetworkHandler.CHANNEL.sendToServer(new StorageAndTakePowerMessage(beacon.getBlockPos(), 1, true));
        }));
        this.addRenderableWidget(new Button(leftPos + 196, topPos + 72, 76, 20, Component.translatable("gui.touhou_little_maid.maid_beacon.min_one"), b -> {
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (potionIndex == -1) {
            ...
            this.drawCenteredStringNoShadow(poseStack, font, Component.translatable("gui.touhou_little_maid.maid_beacon.cost_power", DECIMAL_FORMAT.format(beacon.getEffectCost() * 900)).withStyle(ChatFormatting.RED), leftPos + 195, topPos + 5, 0xffffff);
        }
