...
public class ModelDownloadGui extends Screen {
    ...
    private void addSearchBox() {
        ...
        textField = new EditBox(getMinecraft().font, x + 273, y + 78, 144, 16, TextComponent.EMPTY);
    ...
    private void addPackHandleButtons() {
        if (0 <= this.selectIndex && this.selectIndex < this.showInfos.size()) {
            ...
            this.addRenderableWidget(new FlatColorButton(x + 272, y + 50, 20, 20, TextComponent.EMPTY, b -> openPackWebsite(info))
            ...
            this.addRenderableWidget(new FlatColorButton(x + 398, y + 50, 20, 20, TextComponent.EMPTY, b -> deletePack(info))
                    .setTooltips("gui.touhou_little_maid.resources_download.delete"));
        }
    }

    private void addPageButtons() {
        this.addRenderableWidget(new FlatColorButton(x, y + 218, 40, 20, new TextComponent("<"), b -> {
            if (this.currentPage > 0) {
                this.currentPage--;
                this.init();
            }
        }));
        this.addRenderableWidget(new FlatColorButton(x + 228, y + 218, 40, 20, new TextComponent(">"), b -> {
    ...
    private void addBaseButtons() {
        ...
        for (Condition c : Condition.values()) {
            ...
            Button button = new Button(xPos, y + 3, width, 20, new TranslatableComponent(nameKey), b -> setCondition(c),
                    (b, matrixStack, x, y) -> this.renderTooltip(matrixStack, new TranslatableComponent(descKey), x, y));
        ...
        this.addRenderableWidget(new FlatColorButton(x + 400, y + 2, 20, 20, TextComponent.EMPTY,
        ...
        this.addRenderableWidget(new Button(x + 270, y + 218, 150, 20,
                new TranslatableComponent("gui.touhou_little_maid.resources_download.open_folder"),
    ...
    private void renderNoDataTips(PoseStack poseStack) {
        ...
        List<FormattedCharSequence> split = font.split(new TranslatableComponent("gui.touhou_little_maid.resources_download.fail"), 200);
        int yOffset = y + 100;
    ...
    private void renderPackHandleButtons(PoseStack poseStack) {
        if (0 <= this.selectIndex && this.selectIndex < this.showInfos.size()) {
            DownloadInfo info = this.showInfos.get(this.selectIndex);
            drawCenteredString(poseStack, font, new TranslatableComponent(info.getName()), x + 345, y + 34, 0xffffff);
    ...
    private void renderSearchBox(PoseStack poseStack, int pMouseX, int pMouseY, float pPartialTick) {
        drawString(poseStack, font, new TranslatableComponent("gui.touhou_little_maid.resources_download.hot_search"), x + 274, y + 102, 0xffffff);
        font.drawWordWrap(new TranslatableComponent("gui.touhou_little_maid.resources_download.hot_search_key"), x + 274, y + 115, 146, ChatFormatting.GRAY.getColor());
        textField.render(poseStack, pMouseX, pMouseY, pPartialTick);
        if (textField.getValue().isEmpty() && !textField.isFocused()) {
            drawString(poseStack, font, new TranslatableComponent("gui.touhou_little_maid.resources_download.search").withStyle(ChatFormatting.ITALIC), x + 277, y + 83, 0x777777);
        }
    }
    ...
    public void onClose() {
        if (this.needReload && this.getMinecraft().player != null) {
            this.getMinecraft().gui.setTitle(new TranslatableComponent("gui.touhou_little_maid.resources_download.need_reload.title"));
            this.getMinecraft().gui.setSubtitle(new TranslatableComponent("gui.touhou_little_maid.resources_download.need_reload.subtitle"));
            this.getMinecraft().player.sendMessage(new TranslatableComponent("gui.touhou_little_maid.resources_download.need_reload.subtitle"), Util.NIL_UUID);
        }
        super.onClose();
    }
    ...
    private void deletePack(DownloadInfo info) {
        ...
        if (info.getStatus() == DownloadStatus.DOWNLOADED || info.getStatus() == DownloadStatus.NEED_UPDATE) {
            this.getMinecraft().setScreen(new ConfirmScreen(yes -> this.deleteFilesAndReload(yes, deleteFiles),
                    new TranslatableComponent("gui.touhou_little_maid.resources_download.delete.confirm"),
                    new TranslatableComponent(info.getName())));
        }
    }
