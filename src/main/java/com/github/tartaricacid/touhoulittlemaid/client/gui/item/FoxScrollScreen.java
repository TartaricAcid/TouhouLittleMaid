...
public class FoxScrollScreen extends Screen {
    ...
    public FoxScrollScreen(Map<String, List<FoxScrollMessage.FoxScrollData>> data) {
        super(new TextComponent("Red Fox Scroll"));
        this.data = data;
    ...
    private void addPointButtons() {
        if (StringUtils.isNotBlank(this.selectDim) && this.data.containsKey(this.selectDim)) {
            ...
            if (scrollData.size() > PER_PAGE_COUNT) {
                addRenderableWidget(new FlatColorButton(leftPos + 400 - 20, topPos, 20, 20, new TextComponent("↑"), b -> {
                    if (this.page > 0) {
                        this.page--;
                        this.init();
                    }
                }));
                addRenderableWidget(new FlatColorButton(leftPos + 400 - 20, topPos + 208 - 20, 20, 20, new TextComponent("↓"), b -> {
            ...
            for (int i = this.page * PER_PAGE_COUNT; i < this.page * PER_PAGE_COUNT + PER_PAGE_COUNT; i++) {
                if (i < scrollData.size()) {
                    ...
                    this.addRenderableWidget(new FlatColorButton(leftPos + 400 - 90, offsetIn + 11, 60, 20, new TranslatableComponent("gui.touhou_little_maid.fox_scroll.track"), b -> {
    ...
    private void addDimensionButtons() {
        ...
        for (String dim : this.data.keySet()) {
            Component name = new TextComponent(dim);
    ...
    public void render(PoseStack poseStack, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        if (this.data.isEmpty()) {
            ...
            drawCenteredString(poseStack, font, new TranslatableComponent("gui.touhou_little_maid.fox_scroll.empty"), x, y, 0xFF0000);
            return;
        }
            ...
            for (int i = this.page * PER_PAGE_COUNT; i < this.page * PER_PAGE_COUNT + PER_PAGE_COUNT; i++) {
                if (i < scrollData.size()) {
                    ...
                    if (inSameDim) {
                        ...
                        distanceText = new TranslatableComponent("gui.touhou_little_maid.fox_scroll.distance.same_dimension", distance);
                    } else {
                        distanceText = new TranslatableComponent("gui.touhou_little_maid.fox_scroll.distance.different_dimension");
                    }
                    Component posText = new TranslatableComponent("gui.touhou_little_maid.fox_scroll.position", pos.toShortString());
                    fill(poseStack, leftPos + 152, offsetIn, leftPos + 400 - 22, offsetIn + 40, 0xef58626b);
                    drawString(poseStack, font, info.getName(), leftPos + 160, offsetIn + 4, ChatFormatting.GOLD.getColor());
                    font.draw(poseStack, posText, leftPos + 160, offsetIn + 16, ChatFormatting.GRAY.getColor());
                    font.draw(poseStack, distanceText, leftPos + 160, offsetIn + 28, ChatFormatting.GRAY.getColor());
                    offsetIn = offsetIn + 42;
                }
            }
            if (scrollData.size() > PER_PAGE_COUNT) {
                String pageText = String.format("%d/%d", this.page + 1, (scrollData.size() - 1) / PER_PAGE_COUNT + 1);
                drawCenteredString(poseStack, font, pageText, leftPos + 400 - 8, topPos + 104 - 5, ChatFormatting.GRAY.getColor());
            }
        }
    }
