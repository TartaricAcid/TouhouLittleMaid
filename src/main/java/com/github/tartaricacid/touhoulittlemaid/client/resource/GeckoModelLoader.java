...
public class GeckoModelLoader {
    ...
    private static AnimationFile getAnimationFile(InputStream stream) {
        ...
        JsonObject jsonObject = JSONUtils.fromJson(CustomPackLoader.GSON, new InputStreamReader(stream, StandardCharsets.UTF_8), JsonObject.class);
    ...
    private static void loadDefaultAnimation() {
        try (InputStream stream = Minecraft.getInstance().getResourceManager().getResource(DEFAULT_MAID_ANIMATION).getInputStream()) {
