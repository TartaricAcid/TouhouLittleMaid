...
public final class MobSpawnInfoRegistry {
    ...
    @SubscribeEvent
    public static void addMobSpawnInfo(WorldEvent.PotentialSpawns event) {
        if (event.getWorld() instanceof ServerLevel level) {
            ...
            if (event.getMobCategory() == MobCategory.MONSTER && dimensionIsOkay(dimension)) {
                ...
                long findCount = level.getPoiManager().getInSquare(type -> type.equals(InitPoi.SCARECROW.get()), event.getPos(), scarecrowRange, PoiManager.Occupancy.ANY).count();
                if (findCount <= 0) {
                    event.addSpawnerData(SPAWNER_DATA);
                }
            }
