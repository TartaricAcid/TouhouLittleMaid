...
public class ItemFoxScroll extends Item {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level pLevel, List<Component> components, TooltipFlag pIsAdvanced) {
        ...
        if (info != null) {
            components.add(Component.translatable("tooltips.touhou_little_maid.fox_scroll.dimension", info.getFirst()).withStyle(ChatFormatting.GRAY));
            components.add(Component.translatable("tooltips.touhou_little_maid.fox_scroll.position", info.getSecond().toShortString()).withStyle(ChatFormatting.GRAY));
        }
