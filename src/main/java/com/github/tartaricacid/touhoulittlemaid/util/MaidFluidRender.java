...
public final class MaidFluidRender {
    ...
    public static Component getFluidName(String fluidId, int amount) {
        ...
        if (amount <= 0 || fluid == null || fluid.isSame(Fluids.EMPTY)) {
            return new TranslatableComponent("tooltips.touhou_little_maid.tank_backpack.empty_fluid");
        }
        FluidStack stack = new FluidStack(fluid, amount);
        return stack.getDisplayName();
    }

    public static void drawFluid(PoseStack matrixStack, int x, int y, int width, int height, String fluidId, int amount, int capacity) {
        Fluid fluid = ForgeRegistries.FLUIDS.getValue(new ResourceLocation(fluidId));
        if (amount <= 0 || fluid == null || fluid.isSame(Fluids.EMPTY)) {
            return;
        }
    ...
    private static void drawTiledSprite(PoseStack matrixStack, final int tiledWidth, final int tiledHeight, int color, int scaledAmount, TextureAtlasSprite sprite) {
